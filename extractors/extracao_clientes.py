...
Fluxo:
  ...
  4. Registra o DataFrame no DuckDB e aplica UNNEST de enderecos via SQL
  5. Grava tabela CLIENTES_V1 em Database/VALESOL.duckdb (CREATE OR REPLACE)
  6. Valida e loga a contagem final

Executar:
    py -m extractors.extracao_clientes
    py extractors/extracao_clientes.py
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extracao_clientes.log", encoding="utf-8"),
...
def main():
    ...
    df_addr = pd.json_normalize(df_exploded["enderecos"].tolist())
    ...
    log.info(
        "DataFrame final (apos explode de enderecos): %d linhas x %d colunas",
